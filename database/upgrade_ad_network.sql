...
ALTER TABLE `ads`
  ADD COLUMN `source`     ENUM('upload','embed','vast') NOT NULL DEFAULT 'upload' AFTER `type`,
  ADD COLUMN `media_url`  VARCHAR(500) DEFAULT NULL COMMENT 'remote image or video file' AFTER `media`,
  ADD COLUMN `embed_code` TEXT         DEFAULT NULL COMMENT 'ad network HTML/JS tag' AFTER `body`,
  ADD COLUMN `vast_url`   VARCHAR(500) DEFAULT NULL COMMENT 'VAST/VPAID tag URL' AFTER `embed_code`;
...
('Sample network video (IMA test tag)','video','vast',
